                    ...
                    <div className="pr-1 scrollbar-hide">
                        {!emailSubmitted && !showForgotPassword ? (
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                {/* Strict Signup Redirect for Teacher/Student/Parent */}
                                {!isLogin && (activeRole === UserRole.Teacher || activeRole === UserRole.Student || activeRole === UserRole.Parent) ? (
                                    <div className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg text-center animate-in fade-in slide-in-from-bottom-2">
                                        <div className="mx-auto w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mb-4">
                                            <svg className="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                        </div>
                                        <h4 className="font-bold text-gray-900 mb-2">Dedicated Registration</h4>
                                        <p className="text-sm text-gray-600 mb-6">
                                            Please use the specific registration portal for {activeRole}s to ensure proper account linking and verification.
                                        </p>
                                        <Button
                                            type="button"
                                            onClick={() => navigate(`/signup/${activeRole.toLowerCase()}`)}
                                            className="w-full bg-blue-600 hover:bg-blue-700 text-white shadow-md hover:shadow-lg transition-all"
                                        >
                                            Continue to {activeRole} Signup →
                                        </Button>
                                        <button
                                            type="button"
